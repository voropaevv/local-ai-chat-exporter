...
describe("renderPdf", () => {
  test("leaves visible clearance between code background and a following heading", () => {
    ...
    const background = extractLightCodeBackgrounds(pdfBodyFromBytes(rendered.bytes))[0];
    const heading = extractPdfPositionedTextRuns(rendered.bytes).find(
      (run) => run.text === "Next heading"
    )!;
    const ascent = (new PdfFontRegistry().metrics("bold").capHeight * heading.size) / 1000;
    expect(background.y - (heading.y + ascent)).toBeGreaterThanOrEqual(6);
  });
  test("keeps every line of an oversized table row inside page bounds", () => {
    const rendered = renderPdf(
      makeConversation({
        messages: [
          makeMessage({
            markdown: `| Item | Detail |\n| --- | --- |\n| Long row | ${"Long table content ".repeat(1_000)} |`,
            codeBlocks: []
          })
        ]
      })
    ...
    expect(runs.every((run) => run.y >= 54 && run.y <= 788)).toBe(true);
    expect(
      extractPdfText(rendered.bytes)
        .replace(/\s+/gu, " ")
        .match(/Long table content/gu)?.length
    ).toBe(1_000);
  });
  ...
  test("embeds JPEG images, clickable links, document metadata, and a tagged structure tree", () => {
    ...
    expect(body).toContain(
      "/Title <FEFF0414043e044104420443043f043d044b0439002004300440044504380432>"
    );
  });
