...
LogThread image and ZIP export run locally in the extension. They do not use remote rendering, external export servers, CDN code, telemetry, or broad browser permissions.
...
- If the generated image would exceed that height, LogThread writes a local fallback text file explaining the limit instead of uploading content or using a server renderer.
...
- LogThread does not create a ZIP when every selected export failed or every selected ZIP entry is unavailable.
